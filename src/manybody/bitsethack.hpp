...
#include <iostream> /// REMOVE ME SIMEN KVAAL ! 
...
namespace std
{
  ...
  */
  template<size_t _Nw>
	...
	/// A HACK BY SIMEN KVAAL: comparision support,

      bool
      _M_is_less_than(const _Base_bitset<_Nw>& __x) const
      {
	//std::cerr << "Jaha ... _Nw = " << _Nw << std::endl;
	for (int __i = _Nw-1; __i >=0; --__i)
	  {
	    //std::cerr << this->_M_w[__i] << ", " << __x._M_w[__i] << std::endl;
	    if (_M_w[__i] < __x._M_w[__i])
	      return true;
	    if (_M_w[__i] > __x._M_w[__i])
	      return false;
...
// END OF HACK
  ...
  */
  template<>
	...
	return *new _WordT; 
      ...
      // HACK BY SIMEN KVAAL: comparision. See funny comments above.
      bool
  ...
  */
  template<size_t _Nb>
	...
	/// HACK BY SIMEN KVAAL: comparison support for operator<
